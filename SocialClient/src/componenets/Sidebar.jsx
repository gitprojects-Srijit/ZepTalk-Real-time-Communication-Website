...
const Sidebar = () => {
  ...
  console.log("Sidebar unseenMessages:", unseenMessages);
        ...
        <div className='pb-5'>
          <div className='flex justify-between items-center'>
            <div className='flex items-center justify-center gap-2'>
              <img src="src\assets\ProfileLogo.svg" alt="logo" className='max-w-10'/>
              <p className='text-xl font-bold items-center text-gray-200'>ZapTalk</p>
            </div>
...
group-hover:block'>
          ...
          <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
            ...
            className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search your friend....'/>
          </div>
            ...
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text:sm ${selectedUser ?._id === user._id && 'bg-[#282142]/50'}`}>
              ...
              <div className='flex flex-col leading-5'>
                <p>{user.fullName}</p>
                {
                  ...
                  : <span className='text-orange-500 text-xs'>Offline</span>
                }
              </div>
              {
                (unseenMessages?.[user._id] ?? 0 )> 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex 
